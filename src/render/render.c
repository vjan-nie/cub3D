...
/*   Updated: 2025/12/03 11:35:31 by vjan-nie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * Calculation:
 * y * img->line_len: Bytes needed to skip in order to reach current line.
 * x * (img->bpp / 8): Bytes needed to skip in order to reach the current 'x' pixel.
 * img->data + (...): We add this current pixel position to the begining of the framebuffer: destiny.
 * 
 ...
 * 
 * Color byte storing: '>>' to shift the bits according to the position we're looking for,
 ...
 * using MLX's tool fot that purpose: mlx_new_image(). Each frame will be "pre-drawn"
 * before being loaded as a whole in the window.
